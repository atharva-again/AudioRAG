...
async def _get_youtube_config() -> list[str]:
    """Get YouTube professional setup configuration."""
    env_lines = []
...
async def setup_cmd() -> None:
    ...
    try:
        ...
        youtube_config = await _get_youtube_config()
        env_lines.extend(youtube_config)
...
Configuration:
  AudioRAG uses .env files for configuration. Place a .env file in your
  project directory with AUDIORAG_ prefixed variables (e.g., AUDIORAG_OPENAI_API_KEY).

  The .env file is searched automatically:
