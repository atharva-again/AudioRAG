...
# ============================================================================
# Test AudioSourceRouter
# ============================================================================


class TestAudioSourceRouter:
    """Test suite for AudioSourceRouter URL routing."""

    def test_is_youtube_url_youtube_domains(self):
        """Test YouTube URL detection for various YouTube domains."""
        from audiorag.core.provider_factory import AudioSourceRouter
        ...
        for url in youtube_urls:
            assert AudioSourceRouter._is_youtube_url(url) is True, f"Failed for {url}"

    def test_is_youtube_url_youtu_be(self):
        """Test YouTube URL detection for youtu.be short URLs."""
        from audiorag.core.provider_factory import AudioSourceRouter
    ...
    def test_is_youtube_url_non_youtu_be_subdomain(self):
        """Test YouTube URL detection rejects fake youtu.be subdomains."""
        from audiorag.core.provider_factory import AudioSourceRouter
        ...
        for url in fake_urls:
            assert AudioSourceRouter._is_youtube_url(url) is False, f"Failed for {url}"

    def test_is_youtube_url_non_youtube(self):
        """Test YouTube URL detection rejects non-YouTube URLs."""
        from audiorag.core.provider_factory import AudioSourceRouter

        non_youtube_urls = [
            "https://myyoutube.com/watch?v=abc",
            "https://youtube.com.malicious.com/watch?v=abc",
            "https://myoutu.be/abc",
    ...
    def test_is_youtube_url_file_protocol(self):
        """Test YouTube URL detection with file:// URLs."""
        from audiorag.core.provider_factory import AudioSourceRouter

        file_urls = [
            "file:///home/user/audio.mp3",
            "file:///C:/Users/test/audio.wav",
        ]
    ...
    @pytest.mark.asyncio
    async def test_router_routes_file_urls_to_local(self, tmp_path):
        """Test router routes file:// URLs to LocalSource."""
        from audiorag.core.provider_factory import AudioSourceRouter
        from audiorag.source.local import LocalSource

        # Create a mock local provider
        local_mock = LocalSource()

        # Create router with mock providers
        router = AudioSourceRouter(
            youtube_provider=local_mock,
            local_provider=local_mock,
            url_provider=local_mock,
        )

        # Test routing
        _, name = router._get_provider("file:///home/user/audio.mp3")
        assert name == "local"

    @pytest.mark.asyncio
    async def test_router_routes_youtube_to_youtube(self):
        """Test router routes YouTube URLs to YouTubeSource."""
        from audiorag.core.provider_factory import AudioSourceRouter
        from audiorag.source.youtube import YouTubeSource

        # Create a mock youtube provider
        youtube_mock = YouTubeSource(
        ...
        # Create router with mock providers
        router = AudioSourceRouter(
            youtube_provider=youtube_mock,
            local_provider=youtube_mock,
            url_provider=youtube_mock,
        )

        # Test routing
        _, name = router._get_provider("https://youtube.com/watch?v=abc")
        assert name == "youtube"

    @pytest.mark.asyncio
    async def test_router_routes_http_to_url(self):
        """Test router routes HTTP URLs to URLSource."""
        from audiorag.core.provider_factory import AudioSourceRouter
        ...
        # Create router with mock providers
        router = AudioSourceRouter(
            youtube_provider=url_mock,
            local_provider=url_mock,
            url_provider=url_mock,
        )

        # Test routing
        _, name = router._get_provider("https://example.com/audio.mp3")
        assert name == "url"

    @pytest.mark.asyncio
    async def test_router_conforms_to_protocol(self):
        """Test that AudioSourceRouter conforms to AudioSourceProvider protocol."""
        from audiorag.core.provider_factory import AudioSourceRouter

        router = AudioSourceRouter(
            youtube_provider=None,  # type: ignore[arg-type]
            local_provider=None,  # type: ignore[arg-type]
            url_provider=None,  # type: ignore[arg-type]
        )

        # Router should be an instance of AudioSourceProvider
        assert isinstance(router, AudioSourceProvider)
